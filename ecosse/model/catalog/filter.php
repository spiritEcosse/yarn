...
class ModelCatalogFilter extends Model {
    public function addOption($data) {
        ...
        if (isset($data['option_value'])) {
            foreach ($data['option_value'] as $option_value) {
                ...
                $this->db->query("UPDATE " . DB_PREFIX . "category_option_value_description SET image_option = '" . $option_value['image_option'] . "' WHERE option_id = '" . (int)$option_id . "' AND value_id = '" . (int)$value_id . "' ");
            }
        }
    ...
    public function editOption($option_id, $data) {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query LIKE 'filter=" . (int)$option_id . "%'");
        ...
        if (isset($data['option_value'])) {
            foreach ($data['option_value'] as $value_id => $value) {
                ...
                $this->db->query("UPDATE " . DB_PREFIX . "category_option_value_description SET image_option = '" . $value['image_option'] . "' WHERE option_id = '" . (int)$option_id . "' AND value_id = '" . (int)$value_id . "' ");
            }
        }
        $this->cache->delete('option');
    }
    ...
    public function getOptionValues($option_id) {
        ...
        $value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_option_value cov JOIN " . DB_PREFIX . "category_option_value_description covd ON (cov.value_id = covd.value_id) WHERE cov.option_id = '" . (int)$option_id . "' ORDER BY covd.name = 0, -covd.name, covd.name");
        ...
        foreach ($value_query->rows as $option_value) {
            ...
            foreach ($category_option_value_description->rows as $result) {
                ...
                $image_option = $result['image_option'];
            }

            if ($image_option == '') {
                $image_option = 'no_image.png';
            }
    ...
    public function createTables() {
        ...
        foreach(explode(';', $sql) as $line) {
            $query = str_replace("CREATE TABLE `os_", "CREATE TABLE `" . DB_PREFIX, trim($line));
            if ($query != '') {
                $this->db->query($query);
            }
            $query = '';
    ...
    public function setAliasFromFilter($query, $keyword) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'filter=" . $query . "', keyword = '" . $this->db->escape($keyword) . "'");
    }
