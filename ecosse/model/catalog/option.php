...
class ModelCatalogOption extends Model {
	public function addOption($data) {
        ...
        return array($option_id, $option_value_ids);
	}	
    ...
    public function getOptionByName($option_name) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option_description` od JOIN `" . DB_PREFIX . "option` o ON(od.option_id = o.option_id) WHERE od.name='" . $option_name . "' ");

        return $query->row;
    }

    public function getOptionValueByName($option_id, $option_value_name) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option_value` ov JOIN `" . DB_PREFIX . "option_value_description` ovd ON(ov.option_value_id = ovd.option_value_id) WHERE ovd.name='" . $option_value_name . "' and ov.option_id='" . (int)$option_id . "' ");

        return $query->row;
    }

    public function getOptionValue($option_id, $option_value_id) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option_description` od JOIN `" . DB_PREFIX . "option_value_description` ovd ON (od.option_id=ovd.option_id) WHERE od.option_id = '" . (int)$option_id . "' and ovd.option_value_id='" . (int)$option_value_id . "' ORDER BY ovd.name = 0, -ovd.name, ovd.name");

        return $query->row;
    }

    public function addOptionValue($option_id, $option_value) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option_value_description` ovd where ovd.option_id = '" . (int)$option_id . "', ovd.name='" . $option_value['option_value_description'][1]['name'] . "' ");
	...
	public function getOptionValues($option_id) {
		...
		$option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value ov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE ov.option_id = '" . (int)$option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ovd.name, ov.sort_order ASC");
	...
	public function getOptionValueDescriptions($option_id) {
		...
		$option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value AS t1 JOIN " . DB_PREFIX . "option_value_description AS t2 ON(t1.option_value_id = t2.option_value_id) WHERE t1.option_id = '" . (int)$option_id . "' ORDER BY t2.name, t1.sort_order");
