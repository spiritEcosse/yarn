...
class ControllerCatalogFilter extends Controller {
    ...
    private function getList() {
        ...
        $this->data['filter_alias'] = HTTPS_SERVER . 'index.php?route=catalog/filter/filterToAlias&token=' . $this->session->data['token'];
    ...
    private function getForm() {
        ...
        if (isset($this->request->post['option_values'])) {
            ...
            $option_values = $this->model_catalog_filter->getOptionValues($this->request->get['option_id']);

            foreach ($option_values as $key => $option) {
                if ($option['image_option'] && file_exists(DIR_IMAGE . $option['image_option'])) {
                    $image = $option['image_option'];
                    $option_values[$key]['image_option'] = $image;
                } else {
                    $option_values[$key]['image_option'] = '';
                    $image = 'no_image.jpg';
                }

                $option_values[$key]['thumb'] = $this->model_tool_image->resize($image, 50, 50);
            }
    ...
    public function filterToAlias() {
        $this->load->model('catalog/filter');
        $options = $this->model_catalog_filter->getOptions();

        foreach ($options as $option) {
            foreach ($option['values'] as $option_value) {
                $query = $option['option_id'] . '=' . $option_value['value_id'];
                $keyword = preg_replace('|\s+|', '_', $option['name'] . ' ' . trim($option_value['name']));
                $keyword = str_replace('-', '_', $keyword);
                $keyword = strtolower($this->translitIt($keyword));

                if ($this->model_catalog_filter->getAliasFromFilter($keyword)) {
                    $keyword .= '_' . $option_value['value_id'];
                }

                if (!$this->model_catalog_filter->getAliasFromFilter($keyword)) {
                    $this->model_catalog_filter->setAliasFromFilter($query, $keyword);
                }
            }
        }
