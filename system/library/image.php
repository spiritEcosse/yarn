...
final class Image {
	...
	$watermark = imagecreatefrompng($file);
		...
		imagecolorallocatealpha($watermark, 0, 0, 0, 127);
    	imagecolortransparent($watermark, 80);

        if ($this->info['width'] > 0) {
            for ($count = 1; $count < $this->info['width']; $count++) {
                imagecopy($this->image, $watermark, $watermark_pos_x, $watermark_pos_y, 0, 0, $watermark_width, $watermark_height);
                $watermark_pos_x += $watermark_width;
            }
        } else {
            imagecopy($this->image, $watermark, $watermark_pos_x, $watermark_pos_y, 0, 0, $watermark_width, $watermark_height);
        }
    ...
    public function text($text, $x = 0, $y = 0, $size = 20, $color = '#555') {
		$rgb = $this->html2rgb($color);
