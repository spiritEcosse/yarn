	...
	private function generateTagCloud($tags, $resize = true, $min_font_size = 9, $max_font_size = 25) {
		...
		if ($resize == true){
			...
			foreach ($tags as $key => $value) {
				...
				$cloud[] = '<a onclick="window.location=\'' . (HTTP_SERVER . 'index.php?route=product/search#filterpro_filter_name=' . $key) . '\'" style="text-decoration: none;font-size: ' . $size . 'px" title="' . $value . ' ' .$this->language->get('text_href_title') . ' ' . $key . '">' . $key . '</a> ';
			}
		} else {
			foreach ($tags as $key => $value) {
				$cloud[] = '<a onclick="window.location=\'' . (HTTP_SERVER . 'index.php?route=product/search#filterpro_filter_name=' . $key) . '\'" title="' . $value . ' products tagged with ' . $key . '">' . $key . '</a> ';
			}
		}
