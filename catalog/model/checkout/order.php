...
class ModelCheckoutOrder extends Model {	
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false, $gift = 0) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			$file = '/template/mail/order.tpl';

			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . $file)) {
				$html = $template->fetch($this->config->get('config_template') . $file);
			} else {
				$html = $template->fetch('default' . $file);
			}
			...
			if ($this->config->get('config_alert_mail')) {
				...
				foreach ($order_product_query->rows as $product) {
					$text .= $product['quantity'] . 'x ' . $product['name'] . ' (' . $product['model'] . ') ' . html_entity_decode($this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $product['currency_code'], 1), ENT_NOQUOTES, 'UTF-8') . "\n";
				...
				$file = '/template/mail/admin_order.tpl';
			...
			if ($this->config->get('config_sms_alert')) {
				$options = array(
					...
					array($order_id, date('d.m.Y'), date('H:i'), floatval($order_info['total']), $order_info['telephone']), $this->config->get('config_sms_message'))
				);
