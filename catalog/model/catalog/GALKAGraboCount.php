...
/*------------------------------------------------------------------------
//-----------------------------------------------------
// GraboCountdown Module for GALKA Premium theme by Dimitar Koev 
...
class ModelCatalogGALKAGraboCount extends ModelCatalogProduct {
    ...
    public function getProductSpecialDates($product_id) {
        ...
        $query = $this->db->query("SELECT *, UNIX_TIMESTAMP(date_end) AS date_end_timestamp FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int)$product_id . "' AND customer_group_id = '" . (int)$customer_group_id . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1");
