...
class ModelCatalogRecord extends Model
{
	...
	public function getRecord($record_id) {
		...
		(SELECT price FROM " . DB_PREFIX . "record_discount pd2 WHERE pd2.record_id = p.record_id AND pd2.customer_group_id = '" . (int) $customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount,
		(SELECT price FROM " . DB_PREFIX . "record_special ps WHERE ps.record_id = p.record_id AND ps.customer_group_id = '" . (int) $customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special,
		(SELECT points FROM " . DB_PREFIX . "record_reward pr WHERE pr.record_id = p.record_id AND customer_group_id = '" . (int) $customer_group_id . "') AS reward,
		...
		(SELECT AVG(rating) AS total FROM " . DB_PREFIX . "comment r1 WHERE r1.record_id = p.record_id AND r1.status = '1' GROUP BY r1.record_id) AS rating,
		(SELECT COUNT(*) AS total FROM " . DB_PREFIX . "comment r2 WHERE r2.record_id = p.record_id AND r2.status = '1' GROUP BY r2.record_id) AS comments, p.sort_order 
		...
		WHERE p.record_id = '" . (int) $record_id . "' ". $customer_query." 
	...
	public function searchBlog($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$cache = md5(http_build_query($data));
		$record_data = $this->cache->get('record.search.' . (int) $this->config->get('config_language_id') . '.' . (int) $this->config->get('config_store_id') . '.' . (int) $customer_group_id . '.' . $cache);
		...
		if (!$record_data) {
			...
			foreach ($records as $result) {
				$record_data[$result['record_id']] = $this->getRecordForSearch($result['record_id']);
			}

			$this->cache->set('record.search.' . (int) $this->config->get('config_language_id') . '.' . (int) $this->config->get('config_store_id') . '.' . (int) $customer_group_id . '.' . $cache, $record_data);
		}

		return $record_data;
	}

	public function getRecordForSearch($record_id) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
			$customer_query = " AND (p.customer_group_id = '".(int)$customer_group_id."' OR p.customer_group_id = '".(int)$this->config->get('config_customer_group_id')."')";
		} else {
		...
		(SELECT AVG(rating) AS total FROM " . DB_PREFIX . "comment r1 WHERE r1.record_id = p.record_id AND r1.status = '1' GROUP BY r1.record_id) AS rating,
		(SELECT COUNT(*) AS total FROM " . DB_PREFIX . "comment r2 WHERE r2.record_id = p.record_id AND r2.status = '1' GROUP BY r2.record_id) AS comments, 
		p.sort_order FROM " . DB_PREFIX . "record p
		LEFT JOIN " . DB_PREFIX . "record_description pd ON (p.record_id = pd.record_id)
		LEFT JOIN " . DB_PREFIX . "record_to_store p2s ON (p.record_id = p2s.record_id)
		WHERE p.record_id = '" . (int) $record_id . "' ". $customer_query." 
		AND pd.language_id = '" . (int) $this->config->get('config_language_id') . "' 
	...
	public function general($data = array()) {
		...
		$sql = "SELECT p.record_id,
		(SELECT COUNT(*) AS total FROM " . DB_PREFIX . "comment rc WHERE rc.record_id = p.record_id AND rc.status = '%d' GROUP BY rc.record_id) AS comments,
		(SELECT AVG(rating) AS total FROM " . DB_PREFIX . "comment r1 WHERE r1.record_id = p.record_id AND r1.status = '%d' GROUP BY r1.record_id) AS rating
		FROM " . DB_PREFIX . "record p
		LEFT JOIN " . DB_PREFIX . "record_description pd ON (p.record_id = pd.record_id)
		LEFT JOIN " . DB_PREFIX . "record_to_store p2s ON (p.record_id = p2s.record_id)";
		...
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
			$customer_query = " AND (p.customer_group_id = '%d' OR p.customer_group_id = '%d')";
			$property_data[] = (int)$customer_group_id;
			$property_data[] = (int)$this->config->get('config_customer_group_id');
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
			$customer_query = " AND p.customer_group_id = '%d'";
			$property_data[] = (int)$customer_group_id;
		}
		
		$property_data[] = (int)$this->config->get('config_store_id');

		$sql .= " WHERE pd.language_id = '%d'
		AND p.status = '%d' " . $customer_query . "
		AND NOW() BETWEEN p.date_available AND p.date_end
		AND p2s.store_id = '%d'";
		...
		if (isset($data['filter_name']) && !empty($data['filter_name'])) {
			...
			foreach ($words as $key => $word) {
				...
				$sql .= " OR LCASE(p.model) LIKE '%s'";
				$sql .= " OR LCASE(p.sku) LIKE '%s'";
				$sql .= " OR LCASE(p.upc) LIKE '%s' )";
			}
		}

		if (isset($data['filter_blog_id']) && !empty($data['filter_blog_id'])) {
			if (isset($data['filter_sub_blog']) && !empty($data['filter_sub_blog'])) {
				$implode_data   = array();
				$property_data[] = (int) $data['filter_blog_id'];
				$implode_data[] = "p2c.blog_id = '%d' ";
				$this->load->model('catalog/blog');
				$blogies = $this->model_catalog_blog->getBlogiesByParentId($data['filter_blog_id']);

				foreach ($blogies as $blog_id) {
					$property_data[] = (int)$blog_id;
					$implode_data[] = "p2c.blog_id = '%d' ";
				}
	...
	public function getRecordDiscounts($record_id)
	{
		...
		$query = $this->db->query("
		SELECT * FROM " . DB_PREFIX . "record_discount WHERE record_id = '" . (int) $record_id . "'
		AND customer_group_id = '" . (int) $customer_group_id . "' AND quantity > 1 AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY quantity ASC, priority ASC, price ASC");
		return $query->rows;
	}
	...
	public function getProduct($product_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int) $customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int) $customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW()))
	...
	public function getBlogsRecords($data = array())
	{
		...
		if (!$record_data) {
			$sql = "SELECT p.record_id, p.viewed,
