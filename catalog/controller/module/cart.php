...
class ControllerModuleCart extends Controller {
	public function index() {
		...
		if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					...
					if (isset($total_data[0]['currency'])) {
						$currency_total = $total_data[0]['currency'];
					}
				}
		...
		$this->data['text_items'] = sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), ($currency_total != false) ? $this->currency->format($total, $currency_total, 1) : $this->currency->format($total));
