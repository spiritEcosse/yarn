...
class ControllerModuleGALKAControl extends Controller {
	protected function index() {
		...
		$this->language->load('module/GALKAControl');
...
//Example functionality: pull through customer firstnames and make them available to the view.
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/GALKAControl.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/module/GALKAControl.tpl';
		} else {
			$this->template = 'default/template/module/GALKAControl.tpl';
		}
