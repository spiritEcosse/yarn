...
class ControllerCheckoutCart extends Controller {
    ...
    public function index() {
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            $file = '/template/checkout/cart.tpl';
        } else {
            $this->data['heading_title'] = $this->language->get('heading_title');
    ...
    public function add() {
        ...
        if ($product_info) {
            ...
            if (isset($this->request->post['gift'])) {
                ...
                $discount_birthday_one_gift = $this->config->get('discount_birthday_one_gift');

                if ((int)$discount_birthday_one_gift <= 0) {
                    $discount_birthday_one_gift = 1;
                }

                if ($discount_birthday_one_gift < $quantity) {
                    if ($discount_birthday_one_gift == 1) {
                        $json['error']['gift_quantity'] = sprintf($this->language->get('error_gift_quantity'), $discount_birthday_one_gift);
                    } else {
                        $json['error']['gift_quantity'] = sprintf($this->language->get('error_gift_quantities'), $discount_birthday_one_gift);
                    }
                }
            }
            ...
            if (!$json) {
                $key = $this->cart->add($this->request->post['product_id'], $quantity, $option, $product_price, $gift);
                $json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'path=' . $category_path . '&product_id=' . $product_info['product_id']), $product_info['name'], $this->url->link('checkout/cart'));
                ...
                $json['redirect'] = str_replace('&amp;', '&', $this->url->link('product/product', 'path=' . $category_path . '&product_id=' . $this->request->post['product_id']));
            }
        }
