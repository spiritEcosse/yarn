...
class ControllerAccountCustomerSupport extends Controller {	
    ...
    public function sendMailCustomersInfoCart() {
        $this->reset_cache();
        ...
        foreach ($customers as $customer) {
            ...
            $module['setting']['valid_date_add_cart'] = $this->customer->validDateCartAdd($customer['date_add_cart']);
            $module['setting']['date_add_cart'] = $customer['date_add_cart'];
            $module['setting']['customer_id'] = $customer['customer_id'];
            $module['setting']['cart'] = unserialize($customer['cart']);
            $module['setting']['customer_group_id'] = $customer['customer_group_id'];
            $module['setting']['gift'] = $customer['gift'];
            ...
            $get_html = $this->getChild('account/infocart', $module['setting']);

            if ($get_html) {
                $subject = 'В Вашей корзине есть товары.';

                if (addslashes($this->config->get('subject_user_cart'))) {
                    $subject = addslashes($this->config->get('subject_user_cart'));
                }
