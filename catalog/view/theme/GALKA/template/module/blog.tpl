			...
			<?php foreach ($myblogs as $blogs) { ?>
				...
				<?php for ($i=0; $i<$blogs['flag_start']; $i++) { ?>
					<ul style="padding-left:<?php echo ($blogs['level']*10)+10; ?>px; <?php if(!$blogs['display']) echo 'display:none;' ?>  ">
						<li><a href="<?php if($blogs['active'] == 'active') echo $blogs['href']."#";  else echo $blogs['href']; ?>"
							class="<?php if($blogs['active']=='active') echo 'active'; if($blogs['active']=='pass') echo 'pass'; ?>">
								<?php echo $blogs['name']; if ($blogs['count']>0) echo  " (".$blogs['count'].")"; ?>
							</a>
