...
<?php if ($mycomments) { ?>
    <div class="block_grey">Комментарии / <?php echo count($mycomments); ?></div>

    <?php foreach ($mycomments as $num => $comment) { ?>
        <?php for ($i=0; $i<$comment['flag_start']; $i++) { ?>

<div id="comment-<?php echo $comment['comment_id']; ?>" class="comment_content gradient<?php  echo $comment['level']; ?>" id="" style="overflow: hidden; margin-left: <?php echo ($comment['level'] * 10); ?>px;">
    <div style="padding: 10px;">
        <b><?php  echo $comment['author']; ?></b><br><img src="catalog/view/theme/<?php echo $theme; ?>/image/blogstars-<?php echo $comment['rating'] . '.png'; ?>" alt="<?php  echo $comment['comments']; ?>" />
        ...
        <div style="font-size: 15px; color: <?php if ($comment['delta']>=0) {  echo '#000'; } else {  echo '#AAA'; } ?>;">
            <?php echo html_entity_decode($comment['text'], ENT_QUOTES, 'UTF-8'); ?>
        </div>

        <?php if ($comment['file_comment'] != '') { ?>
            <div class="clear"></div>
            <div class="image">
                <a href="<?php echo $comment['file_comment_popup']; ?>" onclick="return hs.expand(this)"><img src="<?php echo $comment['file_comment']; ?>" /></a>
            </div>
        <?php } ?>
        ...
        <div style="margin-top: 10px; ">
            <a href="#" onclick="comment_reply('<?php echo $comment['comment_id']; ?>'); return false;" id="comment_id_reply_<?php echo $comment['comment_id']; ?>" class="comment_buttons">
                <?php echo $text_reply_button; ?>
            </a>
        </div>
...
<script type="text/javascript">
    ...
    function comment_reply(cid) {
        ...
        $('#comment_work_'+cid).find('textarea[name=\'text\']').attr('id', 'comment_prod_' + cid);
        $('#comment_work_'+cid).find('#file_comment_0').attr('id', 'file_comment_' + cid);
        $('#comment_work_'+cid).find('#file_comment_hidden_0').attr('id', 'file_comment_hidden_' + cid);
        $('#comment_work_'+cid).find('#file_comment_hidden_' + cid).attr('value', '');
        $('#comment_work_'+cid + ' .buttons').find('.info').html('');

        new AjaxUpload($('#file_comment_' + cid), {
            ...
            onSubmit: function(file, extension) {
                $('#file_comment_' + cid).after('<img src="catalog/view/theme/default/image/loading.gif" class="loading" style="padding-left: 5px;" />');
                $('#file_comment_' + cid).attr('disabled', true);
            },
            onComplete: function(file, json) {
                $('#file_comment_' + cid).attr('disabled', false);

                $('.error').remove();

                if (json['success']) {
                    $('#file_comment_hidden_' + cid).attr('value', json['file']);
                    $('#comment_work_' + cid + ' .buttons > .info').html("<?php echo $text_file_upload; ?>: " + json['file_show']);
                }

                if (json['error']) {
                    $('#file_comment_' + cid).after('<span class="error">' + json['error'] + '</span>');
                }
    ...
    function comments_vote(link, comment_id, delta) {
        if($(link).hasClass('loading')) {
            ...
            $.ajax({
                type: 'POST',
                url: 'index.php?route=product/product/comments_vote',
                dataType: 'json',
                async: "false",
